...
6. **One-kick と `--wait` の調停**: `docker compose up -d --wait` は llm の healthy(初回はモデルDL完了)まで返らない。コアパス(読める)はもっと早く成立しているのに、DL失敗時は llm の再起動ループで `--wait` が返らない恐れもある。待つ対象をコアパス4サービスに絞るか、llm の失敗を検知可能にするかの設計判断が未決(2026-07-18 監査で発見)
