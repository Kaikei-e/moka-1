...
4. **単一DB**: PostgreSQL 1インスタンス + pgvector。検索エンジン・キャッシュ・分析基盤は導入しない。全文検索はPostgreSQLの pg_trgm + pgvector ハイブリッドで賄う(素の tsvector FTS は日本語を分かち書きできず実質機能しないため、テキスト側は pg_trgm(標準contrib、イメージ変更不要)を使う — 2026-07-19 決定。PGroonga はカスタムDBイメージが必要になるため不採用)
5. **ローカルLLMのみ**: クラウドAPIキー不要。開発機のiGPU内で完結
...
| `db` | PostgreSQL 18 + pgvector | 記事、フィード、埋め込み、ジョブ状態、すべてここ |
...
**ステップ2までがコアパス、3以降が増強**。イベントソーシング(CQRS)は持たない。スキーマはイミュータブルデータモデル([ADR00002](../adr/ADR00002.md))で、濃縮の成果は INSERT-only のイベント表への追記、pending は成果イベントの不在から導出する(`enrichment_status` カラムは持たない)。
...
|---|---|---|---|
| 高速パス | Qwen3.5-4B (Q4_K_M, ~2.5GB, thinking無効) | ○ | 要約・タグ抽出・翻訳。品質A/BでLFM2.5に p=0.043 で勝ち、実効 6.7s/記事(2.2倍速) |
| 提案層 | LFM2.5-8B-A1B (~4.8GB) / Gemma4 E4B (~3.9GB) / Qwen3.5-4B(兼務) | ○ | 「今日のハイライト」候補を3系統で提案(LFM2.5は網羅性レンズ最強を活かし専任降格) |
| 集約層 | gpt-oss-20B (~11.3GB) | ○ | 提案の審査・統合、RAG Q&Aの最終回答。ウォームロード4-5s |
| (交代枠) | Qwen3.6-35B-A3B (~20.8GB) | スワップ | 品質最優先のバッチ(週次リキャップ等)のみロード(毎回~40s)。リキャップは max_tokens 8192 |
| Embedding | Qwen3-Embedding-0.6B (Q8_0, 0.64GB) | ○ | pgvector 1024次元、クエリ側のみ instruction prefix |

VRAM収支: 常駐合計 ~22GB + KVキャッシュ 3–4GB ≒ 26GB で 48GiB 内(旧記載の「32GB」はBIOS設定変更前の値。現構成ではスワップ枠21GBを載せても ~47GiB で実効55.6GiB内に収まる — eval/ のVRAM実プローブで検証)。モデル選定の根拠と運用注意は §9。

**サービング形態(2026-07-19 決定)**: `llm` コンテナは llama-server の **router mode**(preset ファイルにモデルを宣言、`--models-max` で同時常駐数を制御、モデルごとに子プロセス隔離)1サービスのまま全役割を提供する。コンテナ分割(Tenet 3 抵触)や自前スーパーバイザ entrypoint は不採用。M2 時点の常駐セットは高速パス+Embedding+集約層の3枚。上記VRAM収支は開発機(§冒頭)を**参照構成**とした値であり、より小さいGPUでは router mode の LRU 退避で縮退して動く(OSS利用者への配慮 — フェイルソフトと同じ方向)。pin中の b9859 で embedding モデルが router mode に同居できるかはスパイクで検証し、pin 更新が必要な場合は規約どおり eval/ の実測とセットで行う。
...
**Phase 2 実施記録(2026-07-19)**: セッション認証(HMAC署名cookie検証 — [ADR00021](../adr/ADR00021.md))+レート制限の2フィルタを実装。判明した重要事実: Plecto は Phase 2 時点でも bare `.wasm` をロードせず、**署名済み OCI image-layout + digest pin + `[trust]` 鍵が必須**(fail-closed)のため、Phase 4 予定だった署名・パッケージング工程の一部が前倒しで必要になった(one-shot ビルド/レンダリングジョブ2本で解決、常駐上限外)。`[filter.config]` にシークレットのファイル間接参照が無い問題は manifest レンダリングジョブで回避。フィールドレポートと issue 起票文案6件は `draft_0004_plecto_feedback_phase2.md`(起票はユーザー判断待ち)。
...
```sql
-- リソース(取得時点の事実のみ。状態カラム・updated_at は持たない — ADR00002)
feeds(id, url, title, fetch_interval_seconds, created_at)
articles(id, feed_id, guid, url, title, content, published_at, created_at)
tags(id, name, created_at)

-- イベント(INSERT-only。「状態」は最新イベントから導出)
feed_fetches(...)          -- etag / last_modified / next_fetch はここから導出
enrichment_attempts(...)   -- pending = 成果イベントの不在、backoff = 直近失敗数
article_summaries / article_tags / article_embeddings / article_fulltexts / article_reads
highlights(date, article_ids, rationale, model_meta)
qa_questions / qa_answers  -- 日時属性1つルールで質問と回答を分離
```

リソース3表+イベント群(正確な定義は `db/schema.sql`)。スキーマ管理は **Atlas**([ADR00001](../adr/ADR00001.md)): `db/schema.sql` を単一ソースに `atlas migrate diff` で versioned migrations を生成し、compose 内の one-shot `migrate` ジョブが起動時に自動適用する(One-kick 維持)。moka-core は自前マイグレーション機構を持たない。
...
1. ~~モデル最終選定~~ — [ADR00007](../adr/ADR00007.md)で決着(高速パスは実測A/Bの結果 Qwen3.5-4B に確定、§3.3)
2. ~~集約層の択一~~ — [ADR00007](../adr/ADR00007.md)で決着(gpt-oss-20B 常駐 + Qwen3.6-35B-A3B スワップの二段構え、§3.3)
3. ~~認証方式~~ — 2026-07-19 決定: **パスキー1本 + HMAC署名cookie**。WebAuthn の儀式(登録・ログイン)は moka-core が担い(資格情報はDB保存、BFF経由)、セッションは共有シークレット(Docker secret)による署名cookieで**ステートレス**に発行、Plecto のセッション認証フィルタは署名検証のみ行う(host-kv への書き込み経路問題を回避)。初回登録はパスキー未登録時のみ開放(OSSクローン後の初回ブートストラップ)。詳細はADR化予定
4. **モデル管理UX**: llama.cpp直行方針の下で、pull相当の体験を`compose`内でどう再現するか
5. **フィード取得マナー**: 「外部リクエスト間隔≥5s」のグローバルレートリミッタの具体設計
6. ~~One-kick と `--wait` の調停~~ — router mode 移行([ADR00020](../adr/ADR00020.md))の副産物として解消(2026-07-19)。router の `/health` は「router 生存」を即 200 で返し、モデルDL/ロードは背景で進む(状態は `GET /models` で観測可能)。`--wait` はコアパス成立とほぼ同時に返り、DL失敗もヘルスチェックを巻き込まない(フェイルソフトと同方向)
