...
- **フレッシュ DB で走らせる**(201 / count == 3 / 空状態のアサーションは残留データで壊れる)。
...
```bash
docker compose -f compose.yaml -f compose.e2e.yaml stop e2e-db e2e-migrate e2e-fixtures moka-core moka-web
docker compose -f compose.yaml -f compose.e2e.yaml rm -f e2e-db e2e-migrate e2e-fixtures
```
...
- `e2e-db` / `e2e-migrate` は compose.e2e.yaml でしか定義されない(常駐サービス 5 上限の勘定外、
  tenets §2-3 — e2e-fixtures と同じ扱い)。本番の `db` / `migrate` も e2e 実行時に依存関係上
  一緒に起動しうるが、moka-core は接続しないので触れられない
